'''
Q1 : ベタ書きのままコードをより読みやすくしてください。
※ 2章の設計趣旨に反しますが、APIから取得したデータは問題のためグローバルに置かれるものとします。
'''
...
maps = map(lambda dict_data: tuple(dict_data.keys()), api_get_data)

# 重複タプル除外リストの作成 -> 同一タプルが存在する場合1つだけ残す
unique_tuple_set = set(maps)
unique_tuple_list = list(unique_tuple_set)

# タプルをリストに変換 -> [['id', '名前', '都道府県'], ['id', '名前', '都道府県', 'バルコニー面積'], ['id', '名前']]
maps = map(lambda unique_tuple_data: list(unique_tuple_data), unique_tuple_list)
unique_extraction_key_list = list(maps)
