...
class api_data():
  def __init__(self, api_get_data):
    unique_keys_list = self.get_unique_keys_list(api_get_data)
    self.df = self.transform_from_dict_in_list_to_DataFrame(unique_keys_list)

  def get_unique_keys_list(self, data_list):
  ...
  def transform_from_dict_in_list_to_DataFrame(self, unique_keys_list):
    # 新規データの格納
    data_set = {}
...
api_dataframe = api_data(api_get_data).df